...
function Item({ data, current, setActive, key }: {
    ...
    return (
        <div key={key} onClick={() => setActive()}  className={`${isActive ? " " : "border border-transparent"}`}>
            <div className={`p-5 cursor-pointer border rounded-sm group flex gap-x-4 items-center ${isActive ? "border-2 border-blue-primary " : "border border-gray-300 hover:border-gray-200"}`}>
                <div className="">
