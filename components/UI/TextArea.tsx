...
export default function TextArea(props: TextFieldProps) {
    ...
    return (
        <div className="relative">
            ...
            <textarea rows={props?.rows} className={`w-full py-3 ${props.icon ? "pl-10" : "pl-2"} pr-2 border-gray-200 hover:border-gray-400 focus:border-gray-700 focus:ring-gray-700 focus:ring-opacity-20 focus:ring-3 focus:outline-none border rounded placeholder:text-gray-600`} placeholder={props.label} value={props.value} onChange={(e) => props.setValue(e.target.value)} />
        </div>
    )
}