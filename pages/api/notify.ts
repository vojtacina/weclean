...
export default async function handler(
  ...
  const msg = {
    to: process.env.SENDGRID_MAIL_TO, // Admin email
    from: 'info@focuscleaning.cz', // Change to your verified sender
  ...
  if(data?.contact.includes("@")) {
    const msgToCustomer = {
      to: data.contact, // Customer email
      from: 'info@focuscleaning.cz', // Change to your verified sender
      template_id: "d-2bcbd2fc0702416d85141f9fe9c888a6",
      dynamic_template_data: data
    }

    await sgMail.send(msgToCustomer).then(() => {
